...
class RestClient implements \Iterator, \ArrayAccess {
  ...
  /** @var array  */
	...
	public function __construct($options=array()){
		$default_options = array(
			...
			'base_url' => "https://cms.lingotek.com/api",
      'format' => NULL,
			'format_regex' => "/(\w+)\/(\w+)(;[.+])?/",
			'decoders' => array(
				'json' => 'json_decode',
  ...
  /**
    ...
    $successful_http_codes = array(200, 201);
    if (!in_array($client->info->http_code, $successful_http_codes)) {
      $message = '';
      // special cases
      switch ($client->info->http_code) {
				case 302: $httpcode = 'HTTP/1.1 302 Found'; $message = 'Redirected to: ' . $client->info->redirect_url; break;
        default:
          $message = ' ' . (isset($client->decoded_response->message) && is_array($client->decoded_response->message) ? implode("\n", $client->decoded_response->message) : 'Unknown');
      }

			throw new RestClientException(
				$client->headers->http_code . '. ' . $message
      );
		}
